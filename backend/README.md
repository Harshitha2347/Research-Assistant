...
A full-stack RAG application for chatting with your own PDF library — text **and** figures/charts/diagrams — with hybrid retrieval and a web-search fallback for questions your documents can't answer.
...
3. If the question mentions a figure/chart/diagram/image, candidate figures are shortlisted by keyword overlap against their captions/headings — no API call — and only that shortlist is sent to Gemini vision to describe what's actually in the image.
